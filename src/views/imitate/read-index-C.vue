...
import { getAll, deleteOne } from '@/api/readC'
...
export default {
  ...
  methods: {
    ...
    handleEdit(row) {
      this.$router.push({ name: 'read-main-C', query: { id: row.id }})
    },
    handleVerify(row) {
      this.$router.push({ name: 'read-verify-C', query: { id: row.id }})
    }
  }
}
</script>
