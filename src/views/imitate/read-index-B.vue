...
import { getAll, deleteOne } from '@/api/readB'
...
export default {
  ...
  methods: {
    ...
    handleEdit(row) {
      this.$router.push({ name: 'read-main-B', query: { id: row.id }})
    },
    handleVerify(row) {
      this.$router.push({ name: 'read-verify-B', query: { id: row.id }})
    }
  }
}
</script>
