...
import { getAll, deleteOne } from '@/api/writeA'
...
export default {
  ...
  methods: {
    ...
    handleEdit(row) {
      this.$router.push({ name: 'write-main-A', query: { id: row.id }})
    },
    handleVerify(row) {
      this.$router.push({ name: 'write-verify-A', query: { id: row.id }})
    }
  }
}
</script>
