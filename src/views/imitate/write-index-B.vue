...
import { getAll, deleteOne } from '@/api/writeB'
...
export default {
  ...
  methods: {
    ...
    handleEdit(row) {
      this.$router.push({ name: 'write-main-B', query: { id: row.id }})
    },
    handleVerify(row) {
      this.$router.push({ name: 'write-verify-B', query: { id: row.id }})
    }
  }
}
</script>
