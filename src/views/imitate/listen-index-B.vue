...
import { getAll, deleteOne } from '@/api/listenB'
...
export default {
  ...
  methods: {
    ...
    addOne() {
      this.$router.push('listen-main-B')
    },
    handleSearch(data) {
    ...
    handleEdit(row) {
      this.$router.push({ name: 'listen-main-B', query: { id: row.id }})
    },
    handleVerify(row) {
      this.$router.push({ name: 'listen-verify-B', query: { id: row.id }})
    }
  }
}
</script>
