...
import { getAll, deleteOne } from '@/api/listenA'
...
export default {
  ...
  methods: {
    ...
    addOne() {
      this.$router.push('listen-main-A')
    },
    handleSearch(data) {
    ...
    handleEdit(row) {
      this.$router.push({ name: 'listen-main-A', query: { id: row.id }})
    },
    handleVerify(row) {
      this.$router.push({ name: 'listen-verify-A', query: { id: row.id }})
    }
  }
}
</script>
